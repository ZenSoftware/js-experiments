...
export function dijkstra(from: Vertex, to: Vertex, graph: Vertex[]) {
  ...
  // create unvisited lists
  const unvisited = new Map<Vertex, true>(graph.map(v => [v, true]));

  const unvisitedHeap = new Heap<{ vertex: Vertex; distance: number }>(
    (a, b) => a.distance - b.distance
  );
  for (let v of graph) {
    unvisitedHeap.push({ vertex: v, distance: Infinity });
  }

  while (!unvisitedHeap.isEmpty()) {
    // select the vertex with the shortest recorded distance amongst unvisited
    let shortest = unvisitedHeap.pop()!.vertex;
